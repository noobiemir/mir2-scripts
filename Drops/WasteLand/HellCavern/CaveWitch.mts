import DropList from "../../common.mjs";
const Drop = CS.Daboluo.Mir2.MirScripts.DropInfoDefinition;
const drops = new DropList<CS.Daboluo.Mir2.MirScripts.DropInfoDefinition>();

//钱
drops.Add(new Drop(15000, 1));

//武器
drops.Add(new Drop("镇天", 500))
drops.Add(new Drop("骨玉权杖", 90))
drops.Add(new Drop("无极棍", 90))
drops.Add(new Drop("暗黑刀", 90))
drops.Add(new Drop("FiendBow", 90))
drops.Add(new Drop("龙牙", 145))
drops.Add(new Drop("怒斩", 145))
drops.Add(new Drop("逍遥扇", 145))
drops.Add(new Drop("冷月刀", 145))
drops.Add(new Drop("LitheBow", 145))
drops.Add(new Drop("屠龙", 250))
drops.Add(new Drop("噬魂法杖", 250))
drops.Add(new Drop("龙纹剑", 250))
drops.Add(new Drop("修罗刀", 250))
drops.Add(new Drop("MaleficBow", 250))
drops.Add(new Drop("开天", 500))
drops.Add(new Drop("玄天", 500))
drops.Add(new Drop("暗真魔刀", 500))
drops.Add(new Drop("BowOfForce", 500))
drops.Add(new Drop("裁决之杖", 90))
drops.Add(new Drop("井中月", 75))

//衣服

//头盔
drops.Add(new Drop("黑铁头盔", 30))
drops.Add(new Drop("勇士头盔", 25))
drops.Add(new Drop("英雄头盔", 40))
drops.Add(new Drop("通天冠", 40))
drops.Add(new Drop("修罗头盔", 40))
drops.Add(new Drop("贤者帽", 40))

//项链
drops.Add(new Drop("技巧项链", 500))
drops.Add(new Drop("灵力项链", 10))
drops.Add(new Drop("青雷项链", 10))
drops.Add(new Drop("灵魂项链", 35))
drops.Add(new Drop("恶魔铃铛", 35))
drops.Add(new Drop("绿色项链", 35))
drops.Add(new Drop("狂风项链", 55))

//手镯
drops.Add(new Drop("龙之手镯", 55))
drops.Add(new Drop("骑士手镯", 55))
drops.Add(new Drop("心灵手镯", 55))
drops.Add(new Drop("阎罗手套", 45))
drops.Add(new Drop("黑铁手镯", 95))
drops.Add(new Drop("抗魔轮", 100))
drops.Add(new Drop("伏魔轮", 100))
drops.Add(new Drop("八极轮", 100))

//戒指
drops.Add(new Drop("紫碧螺", 50))
drops.Add(new Drop("屠魔戒指", 30))
drops.Add(new Drop("雪玉戒指", 15))
drops.Add(new Drop("力量戒指", 50))
drops.Add(new Drop("奥玛环", 125))
drops.Add(new Drop("泰坦戒指", 50))
drops.Add(new Drop("心意环", 125))
drops.Add(new Drop("护身戒指", 3000))
drops.Add(new Drop("麻痹戒指", 3000))
drops.Add(new Drop("鬼刃环", 125))

//护身符
drops.Add(new Drop("复活符", 10))

//腰带

//鞋子

//石头

//火把

//药水
drops.Add(new Drop("强效太阳水", 1))
drops.Add(new Drop("魔法药(大量)", 1))
drops.Add(new Drop("金疮药(大量)", 1))
drops.Add(new Drop("魔法药(大量)", 1))
drops.Add(new Drop("金疮药(大量)", 1))
drops.Add(new Drop("金疮药(大量)", 2))
drops.Add(new Drop("强效太阳水", 1))
drops.Add(new Drop("金疮药(大量)", 2))
drops.Add(new Drop("攻击药水(大)", 30))
drops.Add(new Drop("魔法药水(大)", 30))
drops.Add(new Drop("道力药水(大)", 30))
drops.Add(new Drop("疾风药水(大)", 30))
drops.Add(new Drop("太阳水", 2))
drops.Add(new Drop("太阳水", 2))
drops.Add(new Drop("太阳水", 2))
drops.Add(new Drop("太阳水", 2))
drops.Add(new Drop("太阳水", 2))
drops.Add(new Drop("太阳水", 1))
drops.Add(new Drop("太阳水", 1))
drops.Add(new Drop("魔法药(大量)", 2))
drops.Add(new Drop("魔法药(特大)", 2))
drops.Add(new Drop("魔法药(特大)", 2))
drops.Add(new Drop("金疮药(特大)", 2))
drops.Add(new Drop("魔法药(特大)", 1))
drops.Add(new Drop("金疮药(特大)", 1))
drops.Add(new Drop("魔法药(特大)", 1))
drops.Add(new Drop("金疮药(特大)", 1))
drops.Add(new Drop("魔法药(大量)", 2))
drops.Add(new Drop("金疮药(大量)", 2))
drops.Add(new Drop("强效太阳水", 2))
drops.Add(new Drop("强效太阳水", 1))
drops.Add(new Drop("魔法药(大量)", 2))
drops.Add(new Drop("金疮药(特大)", 2))
drops.Add(new Drop("强效太阳水", 2))

//矿石

//肉

//工艺材料

//卷轴
drops.Add(new Drop("祝福油", 10))
drops.Add(new Drop("祝福油", 1))

//宝石
drops.Add(new Drop("疾风神珠", 300))
drops.Add(new Drop("忍耐神珠", 300))
drops.Add(new Drop("集中神珠", 300))
drops.Add(new Drop("酷寒神珠", 300))
drops.Add(new Drop("强化神珠", 200))
drops.Add(new Drop("中毒神珠", 300))
drops.Add(new Drop("守护神珠", 250))
drops.Add(new Drop("仙界神珠", 300))
drops.Add(new Drop("魔性神珠", 300))
drops.Add(new Drop("勇猛神珠", 300))
drops.Add(new Drop("制魔神珠", 250))

//坐骑

//技能书
drops.Add(new Drop("烈火身", 500))
drops.Add(new Drop("复活术", 500))
drops.Add(new Drop("流星火雨", 500))
drops.Add(new Drop("毒云", 500))
drops.Add(new Drop("剑气爆", 500))
drops.Add(new Drop("护身气幕", 500))
drops.Add(new Drop("火龙气焰", 1000))
drops.Add(new Drop("诅咒术", 1000))
drops.Add(new Drop("血风击", 1000))
drops.Add(new Drop("瘟疫", 1000))
drops.Add(new Drop("天霜冰环", 550))
drops.Add(new Drop("吸气", 200))
drops.Add(new Drop("反击", 1000))
drops.Add(new Drop("分身术", 200))
drops.Add(new Drop("召唤月灵", 200))
drops.Add(new Drop("火龙术", 60))
drops.Add(new Drop("无极真气", 60))
drops.Add(new Drop("月影术", 60))
drops.Add(new Drop("圆月弯刀", 55))
drops.Add(new Drop("猛毒剑气", 25))
drops.Add(new Drop("烈火剑法", 25))
drops.Add(new Drop("召唤神兽", 25))
drops.Add(new Drop("冰咆哮", 25))
drops.Add(new Drop("深延术", 1000))
drops.Add(new Drop("攻破斩", 200))

//杂物

//缰绳

//铃铛

//马鞍

//丝带

//面具

//食物

//钩

//浮标

//饵

//探鱼器

//鱼线轮

//鱼

//任务

//觉醒

//宠物

//变身

//其它
drops.Add(new Drop("金条", 60))

export default drops;
