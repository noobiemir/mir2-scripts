import DropList from "../../common.mjs";
const Drop = CS.Daboluo.Mir2.MirScripts.DropInfoDefinition;
const drops = new DropList<CS.Daboluo.Mir2.MirScripts.DropInfoDefinition>();

// 格式:drops.Add(new Drop({物品},{几率}))
// 物品：如输入数字，则为金钱掉落，如输入字符串则为物品掉落
// 几率：物品掉落率为 1 ÷ {几率}，如100则几率为 1%; 400 则为 0.25%

//钱
drops.Add(new Drop(8500, 2));

//武器
drops.Add(new Drop("BowOfForce", 10000))
drops.Add(new Drop("暗真魔刀", 10000))
drops.Add(new Drop("玄天", 10000))
drops.Add(new Drop("镇天", 10000))
drops.Add(new Drop("FiendBow", 250))
drops.Add(new Drop("暗黑刀", 250))
drops.Add(new Drop("无极棍", 250))
drops.Add(new Drop("骨玉权杖", 250))
drops.Add(new Drop("开天", 10000))
drops.Add(new Drop("SilverBow", 100))
drops.Add(new Drop("双极刀", 100))
drops.Add(new Drop("银蛇", 100))
drops.Add(new Drop("魔杖", 100))
drops.Add(new Drop("炼狱", 100))
drops.Add(new Drop("凝霜", 30))
drops.Add(new Drop("修罗", 25))
drops.Add(new Drop("裁决之杖", 250))

//衣服
drops.Add(new Drop("火龙魔衣(男)", 55))
drops.Add(new Drop("火龙魔衣(女)", 55))
drops.Add(new Drop("玄天宝衣(男)", 55))
drops.Add(new Drop("玄天宝衣(女)", 55))
drops.Add(new Drop("StuddedArmour(女)", 55))
drops.Add(new Drop("修罗血衣(女)", 55))
drops.Add(new Drop("StuddedArmour(男)", 55))
drops.Add(new Drop("鬼面甲胄(女)", 55))
drops.Add(new Drop("修罗血衣(男)", 55))
drops.Add(new Drop("鬼面甲胄(男)", 55))
drops.Add(new Drop("幽灵战衣(男)", 30))
drops.Add(new Drop("TemperedArmour(男)", 30))
drops.Add(new Drop("炎红战衣(女)", 30))
drops.Add(new Drop("炎红战衣(男)", 30))
drops.Add(new Drop("幽灵战衣(女)", 30))
drops.Add(new Drop("恶魔长袍(女)", 30))
drops.Add(new Drop("恶魔长袍(男)", 30))
drops.Add(new Drop("战神盔甲(女)", 30))
drops.Add(new Drop("战神盔甲(男)", 30))
drops.Add(new Drop("TemperedArmour(女)", 30))

//头盔
drops.Add(new Drop("道士头盔", 10))
drops.Add(new Drop("骷髅头盔", 15))
drops.Add(new Drop("黑铁头盔", 500))

//项链
drops.Add(new Drop("幽灵项链", 120))
drops.Add(new Drop("天珠项链", 120))
drops.Add(new Drop("生命项链", 120))
drops.Add(new Drop("竹笛", 75))
drops.Add(new Drop("灯笼项链", 10))
drops.Add(new Drop("蓝翡翠项链", 75))
drops.Add(new Drop("白色虎齿项链", 10))
drops.Add(new Drop("放大镜", 75))

//手镯
drops.Add(new Drop("死神手套", 45))
drops.Add(new Drop("金手镯", 25))
drops.Add(new Drop("幽灵手套", 50))

//戒指
drops.Add(new Drop("泰坦戒指", 250))
drops.Add(new Drop("紫碧螺", 250))
drops.Add(new Drop("力量戒指", 250))
drops.Add(new Drop("隐身戒指", 4000))
drops.Add(new Drop("麻痹戒指", 40000))
drops.Add(new Drop("护身戒指", 40000))
drops.Add(new Drop("护身戒指", 40000))
drops.Add(new Drop("治愈戒指", 4000))
drops.Add(new Drop("火焰戒指", 4000))
drops.Add(new Drop("复活戒指", 40000))
drops.Add(new Drop("红宝石戒指", 35))
drops.Add(new Drop("龙之戒指", 35))
drops.Add(new Drop("狂风戒指", 60))
drops.Add(new Drop("珊瑚戒指", 20))
drops.Add(new Drop("降妖除魔戒指", 20))
drops.Add(new Drop("道德戒指", 30))
drops.Add(new Drop("魅力戒指", 30))
drops.Add(new Drop("生铁戒指", 25))
drops.Add(new Drop("骷髅戒指", 25))
drops.Add(new Drop("铂金戒指", 35))

//护身符

//腰带
drops.Add(new Drop("黄金腰带", 550))
drops.Add(new Drop("钢铁腰带", 150))

//鞋子
drops.Add(new Drop("紫绸靴", 300))
drops.Add(new Drop("避魂靴", 700))
drops.Add(new Drop("龙靴", 1200))

//石头
drops.Add(new Drop("道术守护石", 600))
drops.Add(new Drop("魔法守护石", 600))
drops.Add(new Drop("攻击守护石", 600))
drops.Add(new Drop("能力守护石", 600))
drops.Add(new Drop("魔力守护石", 600))
drops.Add(new Drop("生命守护石", 600))
drops.Add(new Drop("魔力守护石(特大)", 250))
drops.Add(new Drop("魔法守护石(特大)", 250))
drops.Add(new Drop("攻击守护石(特大)", 250))
drops.Add(new Drop("能力守护石(特大)", 250))
drops.Add(new Drop("道术守护石(特大)", 250))
drops.Add(new Drop("生命守护石(特大)", 250))

//火把

//药水
drops.Add(new Drop("魔力药水(小)", 30))
drops.Add(new Drop("魔法药水(小)", 30))
drops.Add(new Drop("强效太阳水", 2))
drops.Add(new Drop("强效太阳水", 2))
drops.Add(new Drop("强效太阳水", 2))
drops.Add(new Drop("强效太阳水", 1))
drops.Add(new Drop("强效太阳水", 1))
drops.Add(new Drop("强效太阳水", 1))
drops.Add(new Drop("魔法药(特大)", 2))
drops.Add(new Drop("金疮药(特大)", 2))
drops.Add(new Drop("魔法药(大量)", 2))
drops.Add(new Drop("金疮药(大量)", 2))
drops.Add(new Drop("魔法药(特大)", 2))
drops.Add(new Drop("金疮药(特大)", 2))
drops.Add(new Drop("魔法药(大量)", 2))
drops.Add(new Drop("金疮药(大量)", 2))
drops.Add(new Drop("魔法药(特大)", 1))
drops.Add(new Drop("金疮药(特大)", 1))
drops.Add(new Drop("魔法药(大量)", 1))
drops.Add(new Drop("金疮药(大量)", 1))

//矿石

//肉

//工艺材料

//卷轴

//宝石
drops.Add(new Drop("集中神珠", 2500))
drops.Add(new Drop("回避神珠", 2500))
drops.Add(new Drop("疾风神珠", 2500))
drops.Add(new Drop("酷寒神珠", 2500))
drops.Add(new Drop("中毒神珠", 2500))
drops.Add(new Drop("魔性神珠", 500))
drops.Add(new Drop("制魔神珠", 400))
drops.Add(new Drop("守护神珠", 400))
drops.Add(new Drop("仙界神珠", 500))
drops.Add(new Drop("勇猛神珠", 500))
drops.Add(new Drop("强化神珠", 300))

//坐骑

//技能书
drops.Add(new Drop("迷魂术", 100))
drops.Add(new Drop("无极真气", 60))
drops.Add(new Drop("召唤神兽", 50))
drops.Add(new Drop("复活术", 400))
drops.Add(new Drop("召唤月灵", 400))
drops.Add(new Drop("诅咒术", 400))
drops.Add(new Drop("毒云", 800))
drops.Add(new Drop("阴阳盾", 1200))
drops.Add(new Drop("烈风击", 1200))
drops.Add(new Drop("捕缚术", 60))
drops.Add(new Drop("猛毒剑气", 100))
drops.Add(new Drop("月影术", 100))
drops.Add(new Drop("吸气", 300))
drops.Add(new Drop("烈火身", 400))
drops.Add(new Drop("轻身步", 800))
drops.Add(new Drop("净化术", 50))
drops.Add(new Drop("拔刀术", 275))
drops.Add(new Drop("气功波", 60))
drops.Add(new Drop("野蛮冲撞", 100))
drops.Add(new Drop("天霜冰环", 1200))
drops.Add(new Drop("血风击", 1200))
drops.Add(new Drop("双龙斩", 75))
drops.Add(new Drop("捕绳剑", 150))
drops.Add(new Drop("烈火剑法", 50))
drops.Add(new Drop("狮子吼", 100))
drops.Add(new Drop("圆月弯刀", 60))
drops.Add(new Drop("攻破斩", 100))
drops.Add(new Drop("困魔咒", 100))
drops.Add(new Drop("护身气幕", 400))
drops.Add(new Drop("地狱雷光", 100))
drops.Add(new Drop("冰咆哮", 50))
drops.Add(new Drop("嗜血术", 100))
drops.Add(new Drop("火龙术", 60))
drops.Add(new Drop("分身术", 100))
drops.Add(new Drop("火龙气焰", 400))
drops.Add(new Drop("流星火雨", 800))
drops.Add(new Drop("剑气爆", 800))

//杂物

//缰绳

//铃铛

//马鞍

//丝带

//面具

//食物

//钩

//浮标

//饵

//探鱼器

//鱼线轮

//鱼

//任务

//觉醒

//宠物

//变身

//其它
drops.Add(new Drop("金条", 300))

export default drops;
